...
export const ToolPage = ({ tool }: ToolPageProps) => {
  ...
  const autoFitContent = React.useCallback((elements: unknown) => {
    ...
    try {
      ...
      if (previewElement) {
        (previewElement as HTMLElement).style.transform = "scale(1)";
      }
      ...
      if (hasValidElements && minX !== Infinity) {
        ...
        const padding = 80;
        const availableWidth = containerSize - padding;
        const availableHeight = containerSize - padding;
    ...
    if (excalidrawApi) {
      ...
      if (
        ...
        setTimeout(() => {
          ...
          autoFitContent(sceneData.elements);
